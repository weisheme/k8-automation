...
/* tslint:disable */
/*
function getConfigAndUpsert(owner: string, repo: string, teamId: string, image: string): Promise<void> {
    ...
    return upsertDeployment(config, owner, repo, teamId, image);
}

const i = (process.argv.length > 2) ? process.argv[2] : "gcr.io/reference-implementation-1/atomist-playground/gcb2:0.2.0";
const r = (process.argv.length > 3) ? process.argv[3] : "gcb4";
const o = (process.argv.length > 4) ? process.argv[4] : "atomist-playground";
const t = (process.argv.length > 5) ? process.argv[5] : "AT0M15T";
getConfigAndUpsert(o, r, t, i)
    .then(() => process.exit(0))
    .catch(e => {
