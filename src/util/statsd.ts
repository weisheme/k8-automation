...
export class StatsdAutomationEventListener extends AutomationEventListenerSupport {
    ...
    private registrationName = `${configuration.name}/${configuration.version}`;
...
if (process.env.STATSD_HOST) {
    const options: StatsdOptions = {
        ...
        port: process.env.STATSD_PORT ? parseInt(process.env.STATSD_PORT, 10) : undefined,
    };
    configuration.listeners.push(new StatsdAutomationEventListener(options));
}
